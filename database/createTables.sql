...
CREATE TABLE `fast_task` (
  ...
  `job_result` varchar(3072) default '' COMMENT 'job的结果',
  `job_stat` VARCHAR(1024) DEFAULT NULL COMMENT 'job的聚合统计结果',
...
CREATE TABLE `full_task` (
  ...
  `salt` char(32) NOT NULL COMMENT '全量任务对应的salt，其实就是相对文件夹',
  ...
  `job_stat` VARCHAR(3072) DEFAULT NULL COMMENT 'job的聚合统计结果',
  `status` TINYINT(1) NOT NULL DEFAULT '0' COMMENT 'job当前状态 0=job成功创建,1=job正在执行，2=job运行失败，3=job已经完成，4=线上调度线下成功，5=线上调度线下失败',
